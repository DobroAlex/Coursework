 ...
 *---ECB---
 *  Шифрует каждый блок данных общим ключом, так что паттерн будет заметен на больших объемах данных
 *  Шифрование одинаковых данных одинаковым ключом приведет к одинковому шифротексту. НЕ СТОИТ использовать этот режим без необходимости.
 *  
 *  
 *---CBC---
 *  Шифрует каждый блок в последовательности таким образом, что малейшие изменения данных дадут соврешнно иной шифротекст (эффект лавины)
 *  Также, IV используется так, что шифрование одинковых данных одинаковыми ключами даст разный шифртекст.
 *  Это самый  популярный режим работы
 *  
 *  
 *---CTR---
